...
namespace Bots.DungeonBuddy.Dungeon_Scripts.Mists_of_Pandaria
{
	...
	public class BattleOnTheHighSeasHorde : Dungeon
	{
		...
		public override MoveResult MoveTo(WoWPoint location)
		{
			...
			if (Me.TransportGuid.IsValid || Me.HasAura("Rope Cosmetic"))
			{
				Navigator.Clear();
		...
		private readonly WoWPoint _stage4Loc = new WoWPoint(2167.513, -4231.449, 14.31543);
					...
					new PrioritySelector(
						...
						new Decorator(
							...
							new PrioritySelector(
								...
								new Decorator(
									ctx => !Me.IsCasting,
									new Sequence(new Action(ctx => healObject.Interact()), new WaitContinue(2, ctx => Me.IsCasting, new ActionAlwaysSucceed()))),
		...
		public Func<ScenarioStage, Task<bool>> StageTwoEncounter()
		{
			...
			return async stage =>
			{
				var  barrel = ObjectManager.GetObjectsOfTypeFast<WoWUnit>()
				...
				if (GetBoatFromLocation(Me.Location) == ShipIdentification.CenterAllianceShip)
				{
		...
		public Composite StageThreeEncounter()
		{
			...
			return new PrioritySelector(
				...
				new Decorator(ctx => stage.GetStep(1).IsComplete, new Action(ctx => Navigator.MoveTo(_stage4Loc))));
		}
		...
		public Composite StageFourEncounter()
		{
			return
				new PrioritySelector(
					new Decorator(ctx => GetBoatFromLocation(Me.Location) == ShipIdentification.SouthAllianceShip, ScriptHelpers.CreateClearArea(() => _stage4Loc, 60)),
					new Decorator(
						ctx => Targeting.Instance.IsEmpty() && GetBoatFromLocation(Me.Location) != ShipIdentification.SouthAllianceShip && Me.HealthPercent > 80,
						new Action(ctx => Navigator.MoveTo(_stage4Loc))));
		}
