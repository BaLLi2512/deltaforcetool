		...
		public override void RemoveTargetsFilter(List<WoWObject> units)
		{
			units.RemoveAll(
				ret =>
				{
					...
					if (unit != null)
					{
						if (unit.Entry == JibberwingRiderId && (unit.TransportGuid.IsValid || ScriptHelpers.IsBossAlive("Captain Ook")))
							return true;
					}
					return false;
				});
					...
					keg =
						ObjectManager.ObjectList.Where(
							u => kegs.Contains(u.Entry) && !IsInBlackspot(u.Location) &&
								...
								(u is WoWUnit && !u.ToUnit().TransportGuid.IsValid || u is WoWGameObject && u.ToGameObject().CanUse())).OrderBy(
