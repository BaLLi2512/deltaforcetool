...
namespace Bots.DungeonBuddy.Dungeon_Scripts.Mists_of_Pandaria
{
	...
	public class MogushanPalace : Dungeon
	{
	    ...
	    public override void IncludeLootTargetsFilter(List<WoWObject> incomingObjects, HashSet<WoWObject> outgoingObjects)
	    {
            foreach (var obj in incomingObjects)
            {
                ...
                if (gObj != null)
                {
                    ...
                    if (gObj.Entry == AncientMoguVaultId && !ScriptHelpers.WillPullAggroAtLocation(gObj.Location) && gObj.CanLoot
                        && Me.BagItems.Any(i => i.Entry == AncientMoguKeyId))
                    {
                        outgoingObjects.Add(obj);
                    }
                }
					...
					ctx =>
					{
						...
						if (tankI != null)
						{
							...
							if ((IsOnLift(tank) || _elevatorBottomBoardLoc.Distance(tankLoc) < 8) && !IsOnLift(Me) && tankLevel == myFloorLevel)
							{
								...
								bool elevatorIsReadyToBoard = ele != null && Math.Abs(ele.Z - elevatorRestingZ) <= 2f;
		...
		public bool ElevatorBehavior(WoWPoint destination)
		{
			...
			if (myFloorLevel == 1 && destinationFloorLevel == 2 && IsLastBossDoorOpen)
			{
				...
				if ((ele == null || myloc.DistanceSqr(elevatorBoardLoc) > 20 * 20 ||
					...
					!Me.TransportGuid.IsValid && !Navigator.AtLocation(elevatorBoardLoc)))
				{
				...
				else if (elevatorIsReadyToBoard && myloc.DistanceSqr(elevatorBoardLoc) <= 1.5 * 1.5 && !Me.TransportGuid.IsValid)
				{
					Logger.Write("[Elevator Manager] Jumping");
					WoWMovement.Move(WoWMovement.MovementDirection.JumpAscend);
					WoWMovement.MoveStop(WoWMovement.MovementDirection.JumpAscend);
        ...
        private readonly WoWPoint _ancentMoguTreasureLocation = new WoWPoint(-4397.854, -2563.38, -50.98839);
		...
		public Composite HaiyantheUnstoppableEncounter()
		{
			...
			return new PrioritySelector(
				...
				new Decorator(
					...
					new PrioritySelector(
						new Decorator(
							ctx => boss.CastingSpellId == meteorSpellId && boss.CurrentTargetGuid.IsValid,
							new Action(
								ctx =>
								{
	    ...
	    public async Task<bool> AncientMoguTreasureHandler(WoWGameObject chest)
	    {
            // default loot handler ignores this chest since it can't be navigated to fully but we can still navigate close enough to open it.
	        if (DungeonBuddySettings.Instance.LootMode == LootMode.Off 
                || Blacklist.Contains(chest, BlacklistFlags.Loot) 
                || BotPoi.Current.Type != PoiType.None 
                || !chest.CanUse())
	        {
	            return false;
	        }
            BotPoi.Current = new BotPoi(chest, PoiType.Loot);
	        return true;
	    }
	    ...
	    private const uint AncientMoguVaultId = 214667;
	    private const uint AncientMoguKeyId = 87806;
