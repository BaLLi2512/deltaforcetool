...
namespace Bots.DungeonBuddy.Dungeon_Scripts.Cataclysm
{
    public class VortexPinnacle : Dungeon
    {
        ...
        public override WoWPoint ExitLocation { get { return new WoWPoint(-328.96f, 23.49f, 626.98f); } }
        ...
        public override void WeighTargetsFilter(List<Targeting.TargetPriority> units)
        {
            foreach (var priority in units)
            {
                ...
                if (unit != null)
                {
                    if (unit.Entry == TempleAdeptId && Me.IsDps() &&
                        (Me.IsRange() && !unit.HasAura("Grounding Field") || Me.IsMelee()))
        ...
        private const uint TempleAdeptId = 45935;
        ...
        private static readonly WoWPoint ThrirdAreaSlipStreamLoc1 = new WoWPoint(-733.0685f, 518.6617f, 644.7665f);

        private static readonly WoWPoint[] SlipStreamLocations =
        {
            FirstAreaSlipStreamLoc1, FirstAreaSlipStreamLoc2,
            FirstAreaSlipStreamLoc3, SecondAreaSlipStreamLoc1,
            ThrirdAreaSlipStreamLoc1
        };

        private readonly Vector2[] _firstArea =
        {
        ...
        private readonly Vector2[] _secondArea =
        {
        ...
        private readonly Vector2[] _thirdArea =
        {
		...
		public override async Task<bool> HandleMovement(WoWPoint location)
		{
			...
			if (myArea != destinationArea && myArea != null && destinationArea != null)
			{
				...
				if (myArea == _firstArea)
				{
				    ...
				    {
				        return (await CommonCoroutines.MoveTo(slipStreamLoc)).IsSuccessful();
				    }
				}
				...
				if (myArea == _secondArea)
				{
					...
					else if (destinationArea == _thirdArea)
					{
						...
						{
						    return (await CommonCoroutines.MoveTo(SecondAreaSlipStreamLoc1)).IsSuccessful();
						}
					}
				}
			    if (myArea == _thirdArea)
			    {
			        ...
			        {
			            var slipStream = GetSlipStreamNearLocation(ThrirdAreaSlipStreamLoc1);
			            if (slipStream != null && slipStream.Distance2DSqr <= 20*20)
			            {
			                slipStream.Interact();
			                _slipStreamTimer.Reset();
			            ...
			            {
                            return (await CommonCoroutines.MoveTo(ThrirdAreaSlipStreamLoc1)).IsSuccessful();
			            }
			        }
			    }
				return true;
        ...
        private Vector2[] GetMyArea()
		{
			var myLoc = StyxWoW.Me.Location;

            if (WoWMathHelper.IsPointInPoly(myLoc, _firstArea))
				return _firstArea;

            if (WoWMathHelper.IsPointInPoly(myLoc, _secondArea))
				return _secondArea;

            if (WoWMathHelper.IsPointInPoly(myLoc, _thirdArea))
				return _thirdArea;

			return null;
		}

		private Vector2[] GetDestinationArea(WoWPoint destination)
		{
            if (WoWMathHelper.IsPointInPoly(destination, _firstArea))
				return _firstArea;

            if (WoWMathHelper.IsPointInPoly(destination, _secondArea))
				return _secondArea;

            if (WoWMathHelper.IsPointInPoly(destination, _thirdArea))
				return _thirdArea;
