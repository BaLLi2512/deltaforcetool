	...
	public class ScarletMonastery : Dungeon
	{
        ...
        public override void IncludeLootTargetsFilter(List<WoWObject> incomingObjects, HashSet<WoWObject> outgoingObjects)
        {
            foreach (var obj in incomingObjects)
            {
                ...
                if (gObj != null)
                {
                    if (ScriptHelpers.SupportsQuesting && _bladeoftheAnointedIds.Contains(obj.Entry) 
	    ...
	    private readonly uint[] _bladeoftheAnointedIds =  {214284, 214296};
		...
		{
			return !fuelTank.TransportGuid.IsValid && Me.Combat && await ScriptHelpers.InteractWithObject(fuelTank, 0, true);
		}
