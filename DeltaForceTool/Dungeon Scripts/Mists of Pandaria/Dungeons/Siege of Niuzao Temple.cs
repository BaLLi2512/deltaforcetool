...
namespace Bots.DungeonBuddy.Dungeon_Scripts.Mists_of_Pandaria
{
	public class SiegeOfNiuzaoTemple : Dungeon
	{
		...
		{
			bool vojakArrived = _vojak != null && _vojak.IsValid && Math.Abs(_vojak.Z - 185) < 5;
			units.RemoveAll(
				ret =>
				{
					var unit = ret.ToUnit();
					if (unit != null)
					{
						// Only focus the adds running up until Vojak arrives (still focus them if they are on platform)
						if (_commanderVojakAdds.Contains(unit.Entry) && unit.ZDiff > 5
							&& (Me.IsTank() || vojakArrived))
							return true;
						if (unit.Entry == AmberWingId && (Me.IsMelee() || _doAlternativeVojakStrat))
							return true;
		...
		public override void IncludeTargetsFilter(List<WoWObject> incomingunits, HashSet<WoWObject> outgoingunits)
		{
			foreach (var obj in incomingunits)
			{
				...
				if (unit != null)
				{
					if (unit.Entry == SikthikDemolisherId && !_doAlternativeVojakStrat && unit.Y < 5380 && Me.IsRangeDps())
		...
		public override MoveResult MoveTo(WoWPoint location)
		{
			...
			if (!_doAlternativeVojakStrat && (location.Y >= 5314 || location.Z < 178 || myLoc.Z < 178) 
		...
		readonly WoWPoint _vojakEdgethrowStart = new WoWPoint(1484.795f, 5311.03f, 184.6489f);
		readonly WoWPoint _vojakEdgeThrowEnd = new WoWPoint(1556.34f, 5316.273f, 184.6493f);
		...
		private bool _doAlternativeVojakStrat;
		...
		public Func<WoWUnit, Task<bool>> CommanderVojakEncounter()
		{
			...
			AddAvoidObject(ctx => !Me.IsTank() && !_doAlternativeVojakStrat, 20f, u => u.Entry == SikthikDemolisherId && u.ZDiff <= 5);

			return async boss =>
			{
				var leader = ScriptHelpers.Leader;
				var isLeader = leader != null && leader.IsMe;
				_doAlternativeVojakStrat = leader != null && leader.Z < 165 && leader.Location.DistanceSqr(altStratLoc) < 25*25;
				...
				if (!_doAlternativeVojakStrat && keg != null && !AvoidanceManager.IsRunningOutOfAvoid 
		...
		public Func<WoWUnit,Task<bool>>  WingLeaderNeronokEncounter()
		{
			...
			const int gustingWindsId2 = 121282;
			...
			return async boss =>
			{
				...
				int channelSpell = boss.ChanneledCastingSpellId;
				if (channelSpell == gustingWindsId
					|| channelSpell == gustingWindsId2)
				{
