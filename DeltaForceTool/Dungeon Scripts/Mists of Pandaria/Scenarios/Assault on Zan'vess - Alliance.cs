...
// NOTE: This is the Alliance version..
...
{
	public class AssaultOnZanvessAlliance : Dungeon
	{
		...
		[LocationHandler(-681.848, 4388.878, 186.1933, 100)]
		public Composite CreateGetInCopterBehavior()
		{
			return new PrioritySelector(
				ctx => // set the context to nearest empty copter
				ObjectManager.GetObjectsOfType<WoWUnit>()
							 .Where(u => u.Entry == SkyfireGyrocopterId && Me.PartyMembers.All(p => p.TransportGuid != u.Guid))
							 .OrderBy(u => u.DistanceSqr)
							 .FirstOrDefault(),
				new Decorator<WoWUnit>(
					copter => copter.WithinInteractRange,
					new PrioritySelector(
		...
		public Composite CreateStageTwoBehavior()
		{
			...
			return new PrioritySelector(
				...
				// fire in front of target since it's moving and we're shooting a projectile.
								new Decorator<WoWUnit>(
									tower => tower.TransportGuid.IsValid,
				...
				// shoot at tower now that it's on the ground.
								new Decorator<WoWUnit>(tower => !tower.TransportGuid.IsValid, new Helpers.Action<WoWUnit>(tower => FireBombard(tower.Location))))))),
		...
		public Composite CreateAirstrikeBehavior()
		{
			...
			var firstMissile = WoWMissile.InFlightMissiles.FirstOrDefault();
			var missilePtr = firstMissile.BaseAddress;
