	...
	public class DaggerInTheDarkAlliance : Dungeon
	{
		...
		public override void IncludeTargetsFilter(List<WoWObject> incomingunits, HashSet<WoWObject> outgoingunits)
		{
			foreach (var obj in incomingunits)
			{
				...
				if (unit != null)
				{
					...
					if (Me.TransportGuid.IsValid && Me.Transport.Entry == GoblinCannonId && unit.CanSelect && unit.Attackable && unit.IsHostile && unit.Distance <= 40)
						outgoingunits.Add(unit);
				}
			}
		}
		...
		public override void WeighTargetsFilter(List<Targeting.TargetPriority> units)
		{
			foreach (var priority in units)
			{
				...
				if (unit != null)
				{
					if (unit.Entry == DarkhatchedSorcererId && unit.CastingSpellId == StasicSpellId && Me.TransportGuid.IsValid && Me.Transport.Entry == GoblinCannonId)
						priority.Score += 1000;
				}
			}
		}
		...
		public Composite StageFourEncounter()
		{
			...
			return new PrioritySelector(
				...
				new Decorator(
					ctx =>
					!Me.TransportGuid.IsValid && Me.IsTank() && (boat == null || Me.PartyMembers.All(p => p.TransportGuid != boat.Guid || p.Transport.Entry == GoblinCannonId)),
					new PrioritySelector(
				...
				// boat behavior
				new Decorator(
					ctx => Me.TransportGuid.IsValid && Me.Transport.Entry == GoblinCannonId,
					new PrioritySelector(
						new Decorator(
